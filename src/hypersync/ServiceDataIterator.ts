...
import { IRestDataSetComplete, RestDataSetResult, RestDataSourceBase } from './RestDataSourceBase';
...
export type IteratorPlanDataSetResult = IIteratorPlanComplete | IDataSetResultPending;
...
export const criteriaTransformers: CriteriaTransformers = {
  csvToIterable: (data: any, iterandKey: string) => {
    if (typeof data !== 'string') {
      throw new Error('ServiceDataIterator: CSV criteria data must be a string');
    }
    return data
      .split(',')
      .map((s: string) => s.trim())
...
export class ServiceDataIterator {
  ...
  constructor(restDataSource: IDataSource, dataSetIterator: DataSetIteratorDefinition[], proofType: string) {
    const errMessage = this.validateIterator(ServiceDataIterator.extractIteratorLayer(dataSetIterator, 1));
    if (errMessage) {
      throw new Error(`Iterator: ${errMessage}`);
    }
    if (!isRestDataSourceBase(restDataSource)) {
      throw new Error('ServiceDataIterator: DataSource must be an instance of RestDataSourceBase');
    }
    this.restDataSource = restDataSource;
    this.proofType = proofType;
    this.principalIterator = ServiceDataIterator.extractIteratorLayer(dataSetIterator, 1);
   ...
   */
  public static extractIteratorLayer(dataSetIterator: IProofSpec['dataSetIterator'], layer: number) {
    if (!dataSetIterator || dataSetIterator.length === 0) {
      throw new Error('ServiceDataIterator: dataSetIterator is empty or undefined.');
    }

    const iterator = dataSetIterator.find((i: DataSetIteratorDefinition) => i.layer === layer);
    if (!iterator) {
      throw new Error(`ServiceDataIterator: No matching iterator found for layer: ${layer}`);
    }
    return iterator;
  }
   ...
   */
  public static mergeIterandWithParams(iterand: IterableObject, iterandKey: string, params?: DataValueMap) {
    return {
      ...params,
  ...
  public async generateIteratorPlan(
    ...
    switch (this.principalIterator.source) {
      case IteratorSource.DataSet: {
        const response = await this.generateArrayFromDataset(iteratorParams, metadata);
      ...
      case IteratorSource.Criteria: {
        iterableArray = this.generateArrayFromCriteria(dataSetParams, this.principalIterator.criteriaTransformer);
        break;
      }
      default:
  ...
  public async iterateDataFlow(
    ...
    this.restDataSource.setPagingState(
      this.isSingleIteration() ? PagingState.SingleIteration : PagingState.BatchedIteration
    );
    return this.controlIterativeDataFlow(dataSetName, iterableSlice, params, page, metadata, organization);
  }

  protected generateArrayFromCriteria(params: DataValueMap, transformer?: string) {
    if (!params) {
      throw new Error('ServiceDataIterator: Params are missing.  Unable to generate iterable array from criteria.');
    }
    const criteriaProperty = (this.principalIterator as CriteriaSourcedIterator).criteriaProperty;

    const sourceCriteria = params[criteriaProperty];
    if (!sourceCriteria) {
      throw new Error(`ServiceDataIterator: Missing param value matching criteria property '${criteriaProperty}'`);
    }
    if (!transformer) {
      throw new Error(`ServiceDataIterator: Invalid criteria transformer: ${transformer}`);
    }
    if (
      !Object.prototype.hasOwnProperty.call(criteriaTransformers, transformer) ||
    ...
    const iterableArray = criteriaTransformers[transformer as keyof CriteriaTransformers](
      sourceCriteria,
  ...
  protected async generateArrayFromDataset(params: DataValueMap, metadata?: SyncMetadata) {
    const dataSet = (this.principalIterator as DataSetSourcedIterator).dataSet;
    const response = await this.restDataSource.getData(dataSet!, params, undefined, metadata);
    ...
    if (!Array.isArray(iterableArray)) {
      throw new Error(`ServiceDataIterator: Iterable array of type ${typeof iterableArray} must be an array.`);
    }
  ...
  protected async controlIterativeDataFlow(
    ...
    for (const iteration of iterableSlice) {
      ...
      if (!Array.isArray(response.data)) {
        throw new Error(`ServiceDataIterator: Expected data to be an array, but received: ${typeof response.data}`);
      }
  ...
  protected updateResponseFields(
    ...
    if (response.nextPage && this.isSingleIteration()) accumulator.nextPage = response.nextPage;
   ...
   */
  protected validateIterator(iterator: DataSetIteratorDefinition): string | undefined {
    ...
    switch (iterator.source) {
      ...
      case IteratorSource.Criteria:
        ...
        if (!((iterator as CriteriaSourcedIterator).criteriaTransformer in criteriaTransformers)) {
        ...
        break;
      default:
        return `Invalid iterator source: ${(iterator as DataSetSourcedIterator | CriteriaSourcedIterator).source}`;
    }
    return;
  }
   ...
   * Returns an error message string if validation fails, otherwise undefined.
   */
  protected validateIterableArray(iterableArray: IterableObject[]): string | undefined {
   ...
   * Returns an error message string if validation fails, otherwise undefined.
   */
  protected validateIterableElement(element: IterableObject): string | undefined {
